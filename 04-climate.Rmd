...
taxa_chron <- c("Cupressaceae", "Fraxinus", "Ulmus early", "Pinaceae", "Acer", "Populus", "Quercus", "Betula", "Morus", "Gramineae early", "Gramineae late", "Ulmus late", "Ambrosia")
...
lag_clim_df <- accuracy_df %>%
  ...
  mutate(group = case_when(
    taxa %in% c("Ulmus late", "Gramineae late", "Ambrosia") ~ "late",
    TRUE ~ "early"
  ))

taxa_sig <- lag_clim_df %>%
  group_by(taxa) %>%
...
p_lag_clim <- ggplot(lag_clim_df %>%
  filter(!taxa %in% c("Gramineae early", "Gramineae late", "Ambrosia"))) +
...
```{r,eval=TRUE}
lme.fit <- lme(lag ~ mat, random = ~ 1 + mat | taxa, data = lag_clim_df %>%
  filter(!taxa %in% c("Gramineae early", "Gramineae late", "Ambrosia")) %>%

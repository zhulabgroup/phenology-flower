...
for (siteoi in v_site) {
  ...
  if (nrow (df_doy)>= 500) {
    ...
    ls_df_model[[siteoi]] <- bind_rows(cbind( model_rank$coefficients%>% data.frame() %>%  rownames_to_column(var = "predictor") %>% filter(predictor != "(Intercept)"),

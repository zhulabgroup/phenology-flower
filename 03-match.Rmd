...
Conceptual figure.
```{r, eval=TRUE}
conceptual<-image_read("/raid/users/ysong67/GitHub/phenology/RS4flower/output/figures/conceptual greenup.jpg")
p_conceptual1<-image_ggplot(conceptual)
p_conceptual1
```
...
flower_freq_df_check<-bind_rows(flower_freq_df_check_list)
best_thres<-flower_freq_df_check %>% group_by(taxa,direction, thres) %>% summarise(rmse=median(rmse)) %>% arrange(rmse) %>% slice(1) %>% dplyr::select(taxa,direction, thres)

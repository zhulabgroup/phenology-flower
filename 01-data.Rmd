...
```{r, eval=TRUE}
site_lat<-meta_df %>% filter(site %in% site_list) %>% arrange(lat) %>% pull(sitename)
...
p_nab_ts<-nab_with_taxa_df %>%
  ...
  mutate(taxa_parse=case_when(!taxa%in%c("Cupressaceae", "Pinaceae", "Poaceae")~paste0("italic('",taxa,"')"),
                              TRUE~ taxa)) %>% 
...
p_nab_plant_map<-ggplot() +
  ...
  geom_point(
    data = plant_df %>%
      ...
      group_by(site) %>%
      summarize(midlon=median(lon, na.rm = T),
  ...
  geom_label_repel(data=plant_df %>%
      filter(genus %in% taxa_short_list | family %in% taxa_short_list) %>%
      mutate(taxa = case_when(
        family %in% c("Poaceae", "Cupressaceae", "Pinaceae") ~ family,
        TRUE ~ genus

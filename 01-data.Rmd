...
```{r, eval=TRUE}
taxa_list <- c("Quercus", "Cupressaceae", "Ambrosia", "Morus", "Pinaceae", "Ulmus early", "Ulmus late", "Fraxinus", "Betula", "Gramineae early", "Gramineae late", "Acer", "Populus")
...
p_nab_ts <- nab_with_taxa_df %>%
  ...
  filter(!taxa %in% c("Gramineae", "Ambrosia")) %>%
  ...
  mutate(taxa_parse = case_when(
    !taxa %in% c("Cupressaceae", "Pinaceae", "Gramineae") ~ paste0("italic('", taxa, "')"),
    TRUE ~ taxa
  )) %>%
  ...
  geom_point(
    data = plant_df %>%
      ...
      mutate(taxa = case_when(
        family %in% c("Gramineae", "Cupressaceae", "Pinaceae") ~ family,
        TRUE ~ genus
      )) %>%
      filter(!taxa %in% c("Gramineae", "Ambrosia")) %>%
      group_by(site) %>%
  ...
  iserror <- try(
    for (taxaoi in taxa_list) {
      ...
      for (s in 1:length(site_list)) {
        ...
        if (taxaoi_short %in% c("Ambrosia", "Gramineae")) {
          min_sample_size <- 1
        } else {
          min_sample_size <- 1
        }
...
#   filter(taxa %in% c("Gramineae", "Quercus", "Cupressaceae")) %>%
#            filter(year == 2019) %>%

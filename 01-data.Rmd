...
write_rds(meta_df, "/data/ZHULAB/phenology/nab/meta_dat.rds")
...
p_nab_ts<-nab_with_taxa_df %>%
  ...
  ggplot() +
  geom_tile(aes(x = doy, y = sitename,  fill = count_st), alpha=1) +
  facet_wrap(.~taxa, scales = "free_y") +
  ylab ("Standardized squareroot-transformed pollen count")+
  xlab ("Day of year")+
  theme_classic()+
  ...
  scale_fill_gradient(low = "light yellow", high = "red",na.value="white")
p_nab_ts
```
...
p_nab_plant_map<-ggplot() +
  ...
  geom_point(
    data = plant_df %>%
      ...
      sample_n(100) %>%
      ungroup(),
    aes(x = lon, y = lat), col = "dark blue", cex=0.5,alpha = 0.5
  ) +
  ...
  geom_label_repel(data=meta_df %>% filter(site %in% site_list), aes(x=lon, y=lat, label = sitename),nudge_x = 1.5, nudge_y = 1.5, col="red")+
  coord_equal()
p_nab_plant_map
```
...
registerDoSNOW(cl)
roads_fort_list<-
  foreach (s = 1:length(site_list),
           .packages = c("rgdal", "rgeos", "stringr", "tidyverse", "raster")) %dopar% {
...
roads<-read_rds("/data/ZHULAB/phenology/occurrence/roads/roads_cities.rds")
...
p_plant_map<-ggplot() +
  theme_void() +
  geom_line( data=roads %>% 
               left_join(extent_df, by="site") %>% 
               group_by(site) %>% 
               mutate(long=(long-(maxlon+minlon)/2)/max((maxlon-minlon), (maxlat-minlat)),
  ...
  geom_point(
    data = plant_df %>%
      ...
      left_join(extent_df, by="site") %>% 
      group_by(site) %>% 
      mutate(lon=(lon-(maxlon+minlon)/2)/max((maxlon-minlon), (maxlat-minlat)),
             lat=(lat-(maxlat+minlat)/2)/max((maxlon-minlon), (maxlat-minlat))) %>% 
      ungroup(),
...
write_rds(chelsa_df, "/data/ZHULAB/phenology//nab/chelsa.rds")
```

get_doy<-function (plant_df, ts_df, idoi) {
  ...
  if (length(px_doy [px_doy>=1 & px_doy<=365])<50) {
    ...
    ### green down
    thres_list_down<-thres_df_taxa %>% filter(direction=="down") %>% pull(threshold)
    ...
    ### green up
    thres_list_up<-thres_df_taxa %>% filter(direction=="up") %>% pull(threshold)

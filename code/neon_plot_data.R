...
p_neon_data <- ggplot() +
  geom_point(data = df_neon_evi_sample, aes(x = date, y = evi), alpha = 0.5) +
  geom_line(data = df_neon_evi_sample, aes(x = date, y = evi_sm, group = year), col = "dark green") +
  geom_vline(data = df_neon_doy_sample, aes(xintercept = date), col = "dark green") +
  geom_rect(data = df_neon_doy_sample, aes(xmin = start_date, xmax = end_date, ymin = Inf, ymax = -Inf), fill = "dark green", alpha = 0.25) +
  theme_classic() +

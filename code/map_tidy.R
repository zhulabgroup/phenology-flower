if (!file.exists(str_c(.path$intermediate, "within-urban/df_leaf_pollen_doy.rds"))) {
  ...
  for (siteoi in v_site) {
    ...
    for (yearoi in v_year) {
      ...
      for (taxaoi in v_taxa %>% setdiff("Ulmus late")) {
        ...
        path_tune <- str_c(.path$intermediate, "urban/", taxaoi, "/tune.rds")
  ...
  write_rds(df_doy, str_c(.path$intermediate, "within-urban/df_leaf_pollen_doy.rds"))
} else {
  df_doy <- read_rds(str_c(.path$intermediate, "within-urban/df_leaf_pollen_doy.rds"))
}

      ...
      foreach (i = 1:length(random_id),
               .packages = c("tidyverse", "ptw","greenbrown", "EnvCpt"))  %dopar% {
                 ...
                 if (length(px_doy)<50) {
                   ...
                   px_evi_in_sm<-whit1(px_evi_in,10)
                   px_g2r_in_sm<-whit1(px_g2r_in,10)
                   ...
                   if (!param_ok2) {
                     ...
                     if (length(start_doy:end_doy)<=31 | data_density<1/7) {
                       ...
                       if (length(res2$trendcpt@cpts)<3) {
                         ...
                         flower_cpt<-slope_df %>% pull(cpt)
                         if (length(flower_cpt)==0){
                           flower_cpt<-NA
                         }
                 ...
                 if (debug) {
                   if (length(px_doy)>0) {
                     ...
                     if (length(flower_cpt)==0) {
                       ...
                       p_1tree<-p_1tree+
                         geom_vline(xintercept = flower_cpt, col="red", alpha=0.5)
                     }
                     # if (length(flower_end)==0) {
                     #   p_1tree
                     # } else {
                 ...
                 # data.frame(id=idoi, doy=flower_doy, start=start_doy, end=end_doy, flower_start=flower_start, flower_end=flower_end, sos=sos2)
                 data.frame(id=idoi, start=start_doy, end=end_doy, cpt=flower_cpt, sos=sos2) %>% 
                   mutate(cpt_rel=cpt-sos)
               }
    flower_doy_df<-bind_rows(flower_df_list)
    
    # sos_all<-flower_doy_df %>% drop_na(sos) %>% pull(sos)
    # 
    # ##  Fit bimodal mixture model.
    # yBIC = mclustBIC(sos_all, modelNames="E", G=1:3)
    # yModel = mclustModel(sos_all, yBIC)
    # 
    # ##  Print model parameters.
    # sos_pros<-yModel$parameters$pro
    # sos_centers<-yModel$parameters$mean
    # sos_center<-sos_centers[which(sos_pros==max(sos_pros))]

    # K-Means Cluster Analysis
    cpt_all<-flower_doy_df %>% drop_na(cpt_rel) %>% pull(cpt_rel)

    ##  Fit bimodal mixture model.
    yBIC = mclustBIC(cpt_all, modelNames="E", G=2:5)
    yModel = mclustModel(cpt_all, yBIC)

    ##  Print model parameters.
    cpt_pros<-yModel$parameters$pro
    cpt_centers<-yModel$parameters$mean
    # sos_centers_main<-sos_centers[which(sos_pros==max(sos_pros))]

    fit <- kmeans(cpt_all, centers=cpt_centers, nstart = 5) # 5 cluster solution
    # get cluster means
    cpt_mean<-aggregate(cpt_all,by=list(fit$cluster),FUN=mean) %>%
      arrange(x)

    # append cluster assignment
    cpt_cl <- flower_doy_df %>% drop_na(cpt_rel) %>% 
      mutate(cluster=fit$cluster) 
    ggplot()+
      # geom_histogram(data=flower_doy_df,aes(sos), alpha=0.2, fill="red")+
      geom_histogram(data=cpt_cl, aes(cpt_rel), alpha=0.5)+
      geom_histogram(data=cpt_cl,aes(cpt_rel, col=as.factor(cluster), group=cluster), alpha=0.5)+
      geom_vline(xintercept = 0)

    start_cluster<-1
    end_cluster<-2
    start_mean<-cpt_centers[start_cluster]
    end_mean<-cpt_centers[end_cluster]

    flower_doy_df<-flower_doy_df %>%
      left_join(cpt_cl%>%
                  dplyr::select(id,cpt,cluster)
                  , by=c("id", "cpt")) %>%
      filter(cluster %in% c(start_cluster, end_cluster) & !is.na(cluster)) %>%
      mutate(cluster=case_when(cluster%in%start_cluster~"flower_start",
                            cluster%in%end_cluster~"flower_end")) %>%
      mutate(dist=case_when(cluster=="flower_start"~abs(cpt_rel-start_mean),
                            cluster=="flower_end"~abs(cpt_rel-end_mean))) %>%
      arrange(id, cpt) %>%
      group_by(id, start, end, sos, cluster) %>%
      arrange(dist) %>% 
      slice(1) %>% 
      dplyr::select(-dist, -cpt_rel) %>%
      # summarize(doy=median(doy)) %>%
      spread(key="cluster", value="cpt") %>%
      mutate(doy=(flower_start+flower_end)/2)
      
    flower_doy_df<-flower_doy_df %>% 
      mutate(doy_new=case_when((!doy %in% boxplot.stats(flower_doy_df$doy, coef=1.5)$out)~doy)) %>% 
      mutate(sos_new=case_when((!sos %in% boxplot.stats(flower_doy_df$sos, coef=1.5)$out)~sos)) %>% 

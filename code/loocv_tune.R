for (taxaoi in v_taxa) {
  ...
  if (site_num >= 6) {
    ...
    for (siteout in v_site) {
      ...
      if (siteout %in% (df_tune %>% pull(site) %>% unique())) {
        ...
        if (lag_fit < 0) {
          ...
          df_standard_best_cv <- df_standard_best_cv %>% mutate(freq_sm = replace_na(freq_sm, 0))
        } else if (lag_fit > 0) {
          df_standard_best_cv <- df_standard_best_cv %>%
        ...
        ts_freq_lag_cv <- df_standard_best_cv %>% pull(freq_sm)
        mse_cv <- (mean((ts_freq_lag_cv - ts_pollen_sm_cv)^2, na.rm = T)) # mse between remotely-sensed flowering phenology and pollen count

        df_fit_new[df_fit_new$site == siteout, ]$mse_cv <- mse_cv

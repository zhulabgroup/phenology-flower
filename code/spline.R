...
for (s in 1:length(site_list)) {
  ...
  for (y in 1:length(year_list)) {
    ...
    flower_df_list<-
      foreach (i = 1:length(random_id),
               .packages = c("tidyverse", "ptw","greenbrown", "EnvCpt"))  %dopar% {
                 ...
                 tree_sp<-trees_genus_df %>% filter(id==idoi) %>% pull(species)
                 px_doy<-ts_df_subset %>% filter(id==idoi) %>% filter(var=="G2R (PS)") %>% pull(doy)
                 min_doy_avail<-min(px_doy)
                 if (length(px_doy)<50) {
                   flower_doy<-sos2<-sos1<-start_doy<-end_doy<-NA
                 }  else {
                   ...
                   end_doy<-which(!is.na(px_evi_in_sm)&px_evi_in_sm>=max_evi)[1]+10
                   ...
                   if (!param_ok2) {
                     flower_doy<-start_doy<-end_doy<-sos1<-sos2<-NA
                   } else {
                     
                     sos1<-which(px_evi_in_sm[start_doy:end_doy]>=greenup_thres1) %>% min()+start_doy-1
                     sos2<-which(px_evi_in_sm[start_doy:end_doy]>=greenup_thres2) %>% min()+start_doy-1
                     ...
                     if (length(start_doy:end_doy)<=31 | data_density<1/7) {
                       ...
                       px_doy_new<-start_doy:end_doy
                       px_evi_new<-px_evi_in_sm[start_doy:end_doy]
                       
                       library(segmented)
                       #fit simple linear regression model
                       fit <- lm(px_evi_new~px_doy_new,data=data.frame(px_doy_new,px_evi_new))
                       
                       #fit piecewise regression model to original model, estimating a breakpoint at x=9
                       # set.seed(42)
                       segmented.fit <- segmented(fit, seg.Z = ~px_doy_new, npsi=2, it=10, seed=42)
                       #view summary of segmented model
                       # summary(segmented.fit)
                       # plot(segmented.fit)
                       
                       px_evi_pred<-segmented.fit$fitted.values %>% whit1(30)
                       bp1<-segmented.fit$psi[1,2] 
                       bp2<-segmented.fit$psi[2,2]
                       
                       # peak<-detroit_df_annual %>% filter(id==idoi) %>% pull(peak)
                       
                       fitted_df<-data.frame(px_doy_new,px_evi_new,px_evi_pred ) %>% mutate(anomaly=px_evi_new-px_evi_pred)
                       
                       set.seed(42)
                       res<-envcpt(fitted_df$anomaly, models = c("trendcpt"),verbose = F, minseglen = 5)
                       # plot(res)
                       
                       if (length(res$trendcpt@cpts)<3) {
                         flower_doy<-NA
                       } else {
                         slope_df<-data.frame(cpt=res$trendcpt@cpts,
                                              slope=res$trendcpt@param.est$beta[,2] ) %>%
                 ...
                 if (debug) {
                   if (length(px_doy)>0) {
                     p_1tree<-ggplot(ts_df_subset %>% filter(id==idoi|id=="npn"|id=='pollen') %>% 
                                       filter(var=="EVI (PS)") %>% 
                                       full_join(fitted_df, by=c("doy"="px_doy_new")))+
                       # geom_vline(xintercept = cpt_doy, alpha=0.5, lty=2) +
                       geom_point(aes(doy, value,group=as.factor(id)), col="dark green")+
                       geom_line(aes(doy, px_evi_new), col="dark green")+
                       geom_line(aes(doy, px_evi_pred))+
                       #              aes(doy, q1),alpha=1,
                     ...
                     if (is.na(start_doy)|is.na(end_doy)) {
                       p_1tree
                     } else {
                       p_1tree<-p_1tree+
    ...
    flower_doy_df<-flower_doy_df %>% 
      ...
      mutate(sos_new=case_when((!sos %in% boxplot.stats(flower_doy_df$sos, coef=1)$out)~sos)) %>% 
...
flower_freq_comp<-ggplot(flower_freq_df_standard )+
  ...
  theme_classic()+
  facet_wrap(.~sitename*yearoi, ncol=4, scale="free_x")+
  scale_color_manual(values=cols)+
  theme(legend.position="bottom")+
  theme(legend.title = element_blank())+
  ylab("")
flower_freq_comp
cairo_pdf("./RS4flower/output/figures/flower frequency compared with other data_0407.pdf",
          height = 16, width = 16)

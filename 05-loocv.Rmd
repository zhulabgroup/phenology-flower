...
for (taxaoi in taxa_list) {
  ...
  site_num<-read_rds(paste0(output_path, "accuracy check.rds")) %>% 
    distinct(region) %>% 
    filter(region %in% site_list) %>% 
    nrow()
  ...
  write_rds(lag_clim_df_cv_allsite, paste0(output_path, "climate lag_cv.rds"))
  } else {
...
for (taxaoi in taxa_list) {
  output_path <- paste0("./data/output/", taxaoi, "/")
  output_file<-paste0(output_path, "climate lag_cv.rds")
  if(file.exists(output_file)) {
    lag_clim_df_cv_list[[taxaoi]] <- read_rds(output_file)
  }
}
...
for (siteoutoi in site_list) {
  ...
  early_dat<-lag_clim_df_cv %>%
    filter(siteout == siteoutoi) %>%
    filter(!taxa %in% c("Poaceae early", "Poaceae late", "Ambrosia")) %>%
    filter(group == "early")
  
  if (nrow(early_dat)>0) {
    lme.fit <- lme(lag ~ mat, random = ~ 1 + mat | taxa, data = early_dat, control = lmeControl(opt = "optim", optimMethod = "SANN"))
  p_val_early <- summary(lme.fit)$tTable[2, 5]
  } else {
    p_val_early <- NA
  }
  
  # late_dat<-lag_clim_df_cv %>%
  #   filter(siteout == siteoutoi) %>%
  #   filter(!taxa %in% c("Poaceae early", "Poaceae late", "Ambrosia")) %>%
  #   filter(group == "late") 
  # if (nrow(late_dat)>=1) {
  #   lme.fit <- lme(lag ~ mat, random = ~ 1 + mat | taxa, data = late_dat, control = lmeControl(opt = "optim", optimMethod = "SANN"))
  # p_val_late <- summary(lme.fit)$tTable[2, 5]
  # } else {
  #   p_val_late <- NA
  # }
  
  p_val_df_list[[siteoutoi]] <- data.frame(siteout = siteoutoi, early = p_val_early
                                           # , late = p_val_late
                                           )
}
...
for (taxaoi in taxa_list) {
  output_path <- paste0("./data/output/", taxaoi, "/")
  output_file<-paste0(output_path, "accuracy check_cv.rds")
  if(file.exists(output_file)) {
    loocv_df_list[[taxaoi]] <- read_rds(output_file) %>%
    mutate(taxa = taxaoi)
  }
}
